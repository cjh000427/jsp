	...
	public void execute(HttpServletRequest request, HttpServletResponse response) {
	    ...
	    */
		...
		try {
			...
			if(dao.userCheck(id, pw) == -1) { //아이디 없을때
				htmlCode = "<script>\r\n"
				...
				out.flush();
				out.close();
			} else if(dao.userCheck(id, pw) == 0) { //비번 틀렸을 때
				htmlCode = "<script>\r\n"
	                    + "alert('비밀번호가 틀렸습니다.');\r\n"
	                    + "history.back();\r\n"
	                    + "</script>";
				...
				out.flush();
				out.close();
			} else if(dao.userCheck(id, pw) == 1) { //로그인 성공(아이디, 비번 일치)
